...
- (void)didFailToRegisterForRemoteNotificationsWithError:(NSError*)error {
    ...
    NSLog(@"Failed to register - RESULTS **********%@\n", results);
...
- (void)didReceiveRemoteNotification:(NSDictionary*)userInfo {
	NSLog(@"DidReceiveRemoteNotification while active:%@\n", userInfo);
    
    #if !TARGET_IPHONE_SIMULATOR
    // Here we are receiving the notification and details about it. Since the app is open, we won't display
    // an alert but we will vibrate the device to indicate something happened and set the badge number. You could
    // customize this as desired to also show an alert or not do anything at all..."
        ...
        NSLog(@"Received Push Badge: %@", [apsInfo objectForKey:@"badge"]);
        int badge = [[apsInfo objectForKey:@"badge"] intValue] ?: 0;
...
//	NSString *jsStatement = [NSString stringWithFormat:@"window.plugins.pushNotification.notificationCallback(%@);", [userInfo cdvjk_JSONString]];
    NSString *jsStatement = [NSString stringWithFormat:@"window.pushNotification.notificationCallback(%@);", [userInfo JSONString]];
